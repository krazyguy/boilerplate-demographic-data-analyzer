...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(df['salary'][
    (
        ( df['education']=="Bachelors") |
        ( df['education']=="Masters")  |
        ( df['education']=="Doctorate") 
    )    &     ( df['salary']==">50K") ].count()/higher_education*100,1)

    
    lower_education_rich = round(df['salary'][
    (
        ( df['education']!="Bachelors") &
        ( df['education']!="Masters")  &
        ( df['education']!="Doctorate") 
    )    &     ( df['salary']==">50K")     ].count()/lower_education*100,1)
    ...
    ##making a new df to store total  / rich and calculate percatage for each country
    ...
    country_df["percent-rich"]=round(country_df['rich']/country_df['total']*100,1)

    highest_earning_country = country_df['percent-rich'].idxmax()
    highest_earning_country_percentage = country_df['percent-rich'].max()
